...
async def command_start(message: Message):
  db.db.addUser(message.from_user.id)
...
async def register_task_deadline(message: Message, state: FSMContext):
    inp = message.text
    try:
...
async def register_date_start(message: Message, state: FSMContext):
...
async def register_length(message: Message, state: FSMContext):
    length = message.text
    try:
...
async def command_viewTasks(message: Message):
...
async def command_viewTimetable(message: Message, state: FSMContext):
